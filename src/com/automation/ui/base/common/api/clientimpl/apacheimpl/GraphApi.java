...
public class GraphApi {
    ...
    private static final String SITE_PRODUCTION_APP_ACCESS_TOKEN =
            XMLReader.getValue("ci.user.facebook.prod.accessToken");
    private static final String SITE_PRODUCTION_APP_ID =
    ...
    private static HttpDelete getHttpDelete(URL url) throws URISyntaxException {
        return new HttpDelete(new URIBuilder((new URI(url.getProtocol(), url.getUserInfo(),
                ...
                .addParameter("access_token", SITE_PRODUCTION_APP_ACCESS_TOKEN).build());
    }
    ...
    public FacebookUser createFacebookTestUser() {
        try {
            HttpResponse response = createTestUser(SITE_PRODUCTION_APP_ID);
    ...
    private List<BasicNameValuePair> getParams() {
        return Collections
                .singletonList(new BasicNameValuePair("access_token", SITE_PRODUCTION_APP_ACCESS_TOKEN));
    }
