...
public class IPEBasePageObject extends BasePageObject {


    private static final String LOGGED_IN_USER_SELECTOR =
            ".wds-global-navigation ";

    private static Logger logger = Logger.getLogger(IPEBasePageObject.class);

    @FindBy(css = "#globalNavigation,.wds-global-navigation")
    protected WebElement navigationBar;


    @FindBy(xpath = DeqCommonDetailsCONSTANTS.DEQPROGRESSBAR)
    protected WebElement deqProgressBar;

    public IPEBasePageObject() {
        super();
     }

    //get the progerss bars
    public String getProgressBarStatus( ) {
        return   deqProgressBar.getAttribute("style");
    }
